...
local choice
    while true; do
        choice=$(dialog --colors --backtitle "Select Your Music Choice  BGM Status $bgms  Volume: $vol  Theme: $ts  Music: $ms  Overlay POS: $vpos$hpos  Resolution: $resolution" --title " Music Selection " \
            ...
            2 "Enable/Disable BTTF Music" \
            3 "Enable/Disable Custom Music" \
            4 "Enable/Disable Nostalgia Music" \
            5 "Enable/Disable Supreme Team Music" \
            6 "Enable/Disable Ultimate Vs Fighter Music" \
            7 "Enable/Disable Venom Music" \
            8 "Change Music Folder" \
            9 "Disable Music Folder" \
            2>&1 > /dev/tty)
        case "$choice" in
            1) enable_arcade  ;;
            2) enable_bttf  ;;
